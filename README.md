...
* Provide a one-stop resource for the Node.js developer to get started with sugarchain integration.
* Promote Node.js development of sugarchain web apps.
* Identify and address any incompatibilities with the sugarchains and Bitcoin APIs that exist now and/or in the future.
...
`npm install git://github.com/CryptoLover705/node-sugarchain/`
...
sugarchain.getDifficulty(function() {
...
```js
sugarchain.getNewAddress(function(err, address) {
...
```js
sugarchain.exec('getNewAddress')

sugarchain.exec('getbalance', function(err, balance) {
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a sugarchain address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [sugarchainaddress] </td>
<td> Reveals the private key corresponding to <sugarchainaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current sugarchain address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether sugarchaind is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new sugarchain address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [sugarchainaddress] [minconf=1] </td>
<td> Returns the total amount received by <sugarchainaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute sugarchaind listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [tosugarchainaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<tr>
<td> signmessage </td>
<td> [sugarchainaddress] [message] </td>
...
<td> stop </td>
<td> </td>
<td> Stop sugarchain server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [sugarchainaddress] </td>
<td> Return information about [sugarchainaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [sugarchainaddress] [signature] [message] </td>
...
var sugarchain = require('sugarchain')({
    user:'user'
})

sugarchain.set('pass', 'somn')
sugarchain.set({port:7142})
...
You may pass an optional function `passphrasecallback` to the `node-sugarchain` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `sugarchaind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `sugarchaind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `sugarchain.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/sugarchaind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/sugarchaind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `sugarchaind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-sugarchain is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('sugarchaind.crt')
